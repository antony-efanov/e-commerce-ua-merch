...
function Card({ id, parentID, imgUrl, title, price, amount, onClickFav, onClickPlus, inCart = false, loading = false }) {

  let favID = `${id}_fav`;
  let checkID = `${id}_check`;

  const onClickFavBtn = () => {
    onClickFav({imgUrl, title, price})
  }
    ...
    <>
    {
      loading ? (
        ...
        <div className="card">
          ...
          <label onClick={onClickFavBtn} htmlFor={favID} className="card__favbox--btn">
