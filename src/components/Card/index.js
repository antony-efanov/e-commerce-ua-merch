...
function Card({ id, parentID, imgUrl, title, price, amount, onClickFav, onClickPlus, inCart = false, inFav = false, loading = false }) {

  let favID = `${id}_fav`;
  let checkID = `${id}_check`;

  const onFav = () => {
    onClickFav({id, parentID, imgUrl, title, price, amount})
  }

  const onPlus = () => {
    onClickPlus({id, parentID, imgUrl, title, price, amount});
  }
  ...
  return (
    <>
    {
      loading ? (
        ...
        >
            <rect rx="10" ry="10" width="100%" height="65%" />
            <rect x="0" y="105" rx="5" ry="5" width="100%" height="15" /> 
            <rect x="0" y="127" rx="5" ry="5" width="40%" height="18" />
            <rect x="80" y="127" rx="5" ry="5" width="20%" height="18" /> 
        </ContentLoader>
        ) : (
        <div className="card">
          <p 
          className={availabilityClass()}>{availability()}</p>
          <input defaultChecked={inFav} type="checkbox" className='card__favbox' id={favID}/>
          <label onClick={onFav} htmlFor={favID} className="card__favbox--btn">
          ...
          <div className="card__text">
            ...
            {amount !== "0" && 
              <div>
                <input defaultChecked={inCart} type="checkbox" className='card__checkbox' id={checkID}/>
                <label onClick={onPlus} htmlFor={checkID} className="card__checkbox--btn"></label>
              </div>}
            </div>
          </div>
        </div>
