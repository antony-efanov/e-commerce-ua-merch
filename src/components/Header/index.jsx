...
export function Header({ onClickCart }) {
  ...
  const { cartItems, favItems, cartVisibility, setCartVisibility } = useContext(AppContext);
  ...
  return (
    <header className="header">
      ...
      <div className="user">
        <button onClick={onCart} className={cartVisibility ? "user__btn user__btn--cart user__btn--cart__active" : "user__btn user__btn--cart"}>
          <svg width="25" height="25" viewBox="0 0 20 20" fill="none">
            ...
            <path d="M1 1H4.27273L6.46545 11.9555C6.54027 12.3321 6.7452 12.6705 7.04436 12.9113C7.34351 13.1522 7.71784 13.2801 8.10182 13.2727H16.0545C16.4385 13.2801 16.8129 13.1522 17.112 12.9113C17.4112 12.6705 17.6161 12.3321 17.6909 11.9555L19 5.09091H5.09091" stroke="#9B9B9B" strokeWidth="2" strokewinecap="round" strokeLinejoin="round"/>
          </svg>
          {cartItems.length > 0 && <div className='cartCounter'><p>{cartItems.length}</p></div>}
        </button>
        <Link to="/e-commerce-ua-merch/favorite" onClick={onFav} className={favOpened ? "user__btn user__btn--fav__active" : "user__btn user__btn--fav"}>
          ...
          {favItems.length > 0 && <div className='favCounter'><p>{favItems.length}</p></div>}
        </Link>
        <Link to="/e-commerce-ua-merch/orders" onClick={onOrders} className={ordersOpened ? "user__btn user__btn--user__active" : "user__btn user__btn--user"}>
          <svg width="25" height="25" viewBox="0 0 20 20" fill="none">
